...
void wifi_scan_and_display(lv_obj_t *dropdown) {
    // Simulación de redes Wi-Fi encontradas
    const char *networks[] = {"Red1", "Red2", "Red3", "Red4"};
    lv_dropdown_clear_options(dropdown);
    for (int i = 0; i < 4; i++) {
        lv_dropdown_add_option(dropdown, networks[i], LV_DROPDOWN_POS_LAST);
    }
}
...
void create_wifi_settings_widget(lv_disp_t *disp) {
    ...
    // Dropdown para SSIDs
    lv_obj_t *ssid_dropdown = lv_dropdown_create(container);
    lv_obj_set_width(ssid_dropdown, 200);
    wifi_scan_and_display(ssid_dropdown);
    ...
    // Evento de conectar
    lv_obj_add_event_cb(connect_btn, back_button_event_handler, LV_EVENT_CLICKED, NULL);
}

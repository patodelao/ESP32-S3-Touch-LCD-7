...
// Función para crear el header común
lv_obj_t * create_header(lv_obj_t * parent) {
    ...
    lv_label_set_text(title, "Mi Aplicación - Estado WiFi | Hora");
    lv_obj_center(title);
    
    return header;
}

// Función para crear la pantalla base con header y contenedor de contenido
lv_obj_t * create_base_screen(void) {
    // Creamos el contenedor base que abarca la pantalla completa
    lv_obj_t * base = lv_obj_create(lv_scr_act());
    lv_obj_set_size(base, lv_disp_get_hor_res(NULL), lv_disp_get_ver_res(NULL));
    ...
    lv_obj_set_size(content, 800, 450); // 480 - 50 = 430
    lv_obj_clear_flag(content, LV_OBJ_FLAG_SCROLLABLE);

    // Alineamos el contenedor de contenido justo debajo del header
    lv_obj_align(content, LV_ALIGN_CENTER, 0, 50);
...
static void back_btn_event_cb(lv_event_t * e);
 ...
 static void delete_last_item(lv_event_t * e) {
    ...
    lv_obj_del(cont_arr[cont_index - 1]); // Elimina el último contenedor
...
char *load_products_from_nvs() {
    ...
    err = nvs_get_str(my_handle, "products", NULL, &required_size);
    
    if (err == ESP_OK && required_size > 0) {
        char *products = malloc(required_size);
        if (products == NULL) {
            ESP_LOGE(TAG, "Error al asignar memoria para productos");
            nvs_close(my_handle);
            return NULL;
        }

        nvs_get_str(my_handle, "products", products, &required_size);
        nvs_close(my_handle);
        return products; // Devuelve la cadena con los productos
    }

    nvs_close(my_handle);
    return strdup("No hay productos guardados"); // Retorna mensaje por defecto si no hay productos
}
 ...
 static void create_new_product(lv_event_t * e) {
    ...
    snprintf(default_name, sizeof(default_name), "Producto %" PRIu32, cont_index +1);
...
static void transition_to_main_screen(void * param) {
    ...
    create_main_screen();         // Crea el menú principal
}

static void go_to_main_screen(lv_event_t * e) {
    save_products_to_nvs();         // Guarda los productos antes de salir
    lv_async_call(transition_to_main_screen, NULL);  // Llama asíncronamente a la transición
}
...
void load_products_for_config(void) {
    ...
    // Reiniciamos el índice de contenedores
    cont_index = 0;
    ...
    for (uint32_t i = 0; i < saved_product_count; i++) {
        ...
        if (nvs_get_str(my_handle, key, product_name, &len) == ESP_OK) {
            // Crear la sub-página para este producto (igual que en create_new_product)
            lv_obj_t * new_sub_page = lv_menu_page_create(menu, NULL);
            lv_obj_add_event_cb(new_sub_page, sub_page_event_cb, LV_EVENT_ALL, NULL);

            // Crear los tres campos de texto en la sub-página
            lv_obj_t * name_ta = create_textarea(new_sub_page, "Nombre");
            // Asignar el nombre cargado
            lv_textarea_set_text(name_ta, product_name);

            lv_obj_t * price_ta = create_textarea(new_sub_page, "Precio");
            // Puedes cargar un valor guardado para el precio, o dejarlo vacío
            lv_textarea_set_text(price_ta, "");

            lv_obj_t * desc_ta = create_textarea(new_sub_page, "Descripción");
            // Igual para la descripción
            lv_textarea_set_text(desc_ta, "");

            // Crear el botón de guardar en la sub-página
            lv_obj_t * save_btn = lv_btn_create(new_sub_page);
            lv_obj_t * save_label = lv_label_create(save_btn);
            lv_label_set_text(save_label, "Guardar");
            lv_obj_add_event_cb(save_btn, save_button_event_cb, LV_EVENT_CLICKED, new_sub_page);

            // Crear el contenedor interactivo en la página principal del menú
            lv_obj_t * cont = lv_menu_cont_create(main_page);
            lv_obj_t * cont_label = lv_label_create(cont);
            // Usar el nombre del producto cargado en el contenedor
            lv_label_set_text(cont_label, product_name);
            // Vincular el contenedor con su sub-página
            lv_menu_set_load_page_event(menu, cont, new_sub_page);

            // Guardar la referencia bidireccional
            lv_obj_set_user_data(cont, new_sub_page);
            lv_obj_set_user_data(new_sub_page, cont);

            lv_obj_scroll_to_view_recursive(cont, LV_ANIM_ON);

            // Guardar el contenedor en el arreglo global y actualizar el índice
            cont_arr[cont_index] = cont;
            cont_index++;
        }
    }
...
void product_config(void){
    ...
    /* Crear un contenedor principal para toda la configuración */
    lv_obj_t *config_container = lv_obj_create(lv_scr_act());
    ...
    // Opcional: aplicar un fondo o borde para distinguirlo del fondo principal
    ...
    lv_obj_set_size(header, 750, 60);  // Altura del header
    lv_obj_align(header, LV_ALIGN_TOP_MID, 0, -25);              // Ubicar en la parte superior
    lv_obj_clear_flag(header, LV_OBJ_FLAG_SCROLLABLE);
    ...
    /* --- MENÚ --- */
    /* Crear un contenedor para el menú (área entre header y footer) */
    lv_obj_t *menu_container = lv_obj_create(config_container);
    lv_obj_set_size(menu_container, 750 ,350); // espacio entre header y footer
    lv_obj_align(menu_container, LV_ALIGN_TOP_MID, 0, 25);
    lv_obj_set_scroll_dir(menu_container, LV_DIR_VER);
    lv_obj_set_scrollbar_mode(menu_container, LV_SCROLLBAR_MODE_OFF); // Desactivar scrollbar

    /* Crear el menú dentro del contenedor scrolleable */
    menu = lv_menu_create(menu_container);
    lv_obj_clear_flag(menu, LV_OBJ_FLAG_SCROLLABLE);    // Desactivar scroll
    lv_obj_set_size(menu, 800, 300 );
    lv_obj_align(menu, LV_ALIGN_TOP_MID, 50, 0);

    /* Crear la página principal del menú y asignarla */
    main_page = lv_menu_page_create(menu, NULL);
    lv_menu_set_page(menu, main_page);

    load_products_for_config(); // Carga los productos guardados en NVS

    /* Agregar evento para el botón de retroceso en la página principal */
    lv_obj_t *back_btn_menu = lv_menu_get_cur_main_page(menu);
    lv_obj_add_event_cb(back_btn_menu, back_btn_event_cb, LV_EVENT_CLICKED, NULL);
    ...
    lv_obj_align(float_btn_add, LV_ALIGN_RIGHT_MID, -30, 0);
    ...
    lv_obj_align(float_btn_del, LV_ALIGN_RIGHT_MID, -90, 0);
    ...
    lv_obj_align(float_btn_del_all, LV_ALIGN_RIGHT_MID, -140, 0);
...
////////////////////////////////
 /*-----------------------------------------------------------
 * Función que crea la interfaz de configuración de productos
 * dentro de un contenedor sobre el objeto "parent" (por ejemplo,
 * el contenido de una pestaña).
 *----------------------------------------------------------*/

 static void product_config_in_tab(lv_obj_t *parent) {
    /* Asegurarse de que la pestaña (parent) tenga un tamaño correcto.
       En este ejemplo lo forzamos a 800x480. */
    ...
    lv_obj_set_size(config_container, 740, 470);
    //lv_obj_set_size(config_container,lv_disp_get_hor_res(NULL),lv_disp_get_ver_res(NULL));
    lv_obj_align(config_container, LV_ALIGN_CENTER, 60, 5);
    lv_obj_set_style_bg_color(config_container, lv_color_make(240,240,240), 0);
    ...
    /* --- MENÚ --- */
    lv_obj_t *menu_container = lv_obj_create(config_container);
    lv_obj_set_size(menu_container, 800, 480 - 50 - 60);  // Resta header (50) y footer (60)
    ...
    lv_obj_set_size(menu, 800, lv_obj_get_height(menu_container));
    ...
    /* Cargar los productos guardados en NVS (si los hubiera) */
...
/*-----------------------------------------------------------
 * Función que crea un tabview con 3 pestañas.
 * En la segunda pestaña se monta la configuración de productos.
 *----------------------------------------------------------*/
...
void create_general_config_screen(void)
{
    ...
    lv_obj_t *tabview = lv_tabview_create(lv_scr_act(), LV_DIR_LEFT, 60);
    ...
    /* Agregar contenido a Tab 1 y Tab 3 (puedes personalizarlos) */
    lv_obj_t *label = lv_label_create(tab1);
    lv_label_set_text(label, "Contenido de Tab 1");

    /* En Tab 2 se monta la configuración de productos */
...
void create_main_screen(void) {
    // Obtenemos la pantalla activa
    lv_obj_t * scr = lv_scr_act();

    // 1) Creamos el header directamente en la pantalla
    lv_obj_t * header = create_header(scr);

    // 2) Creamos el contenedor para el contenido y lo posicionamos debajo del header
    lv_obj_t * content = lv_obj_create(scr);
    lv_obj_clear_flag(content, LV_OBJ_FLAG_SCROLLABLE);    // No permitir scroll
    lv_obj_set_size(content, lv_disp_get_hor_res(NULL), lv_disp_get_ver_res(NULL) - 30);
    lv_obj_align(content, LV_ALIGN_TOP_MID, 0, 30);

    // 3) Creamos el panel principal para el contenido
    // **Panel Exhibidor de Productos**
    lv_obj_t *product_panel = lv_obj_create(content);
    lv_obj_set_size(product_panel, 800, 250);
    lv_obj_clear_flag(product_panel, LV_OBJ_FLAG_SCROLLABLE);    // No permitir scroll
    ...
    // Configurar layout FLEX para distribuir los productos equitativamente y centrarlos
    lv_obj_set_layout(product_panel, LV_LAYOUT_FLEX);
    lv_obj_set_style_flex_flow(product_panel, LV_FLEX_FLOW_ROW, 0);
    // Centra los items tanto en el eje principal como en el cruzado
        ...
        if (product_count > 0) {
            for (uint32_t i = 0; i < product_count; i++) {
                ...
                if (nvs_get_str(my_handle, key, product_name, &length) == ESP_OK) {
                    ...
                    lv_obj_clear_flag(item, LV_OBJ_FLAG_SCROLLABLE);    // No permitir scroll
                    lv_obj_set_size(item, 120, 100);
                    lv_obj_set_style_bg_color(item, lv_palette_main(LV_PALETTE_BLUE), 0);
                    // ¡No se usa lv_obj_align() aquí! El layout FLEX se encargará de posicionar el item.
    ...
    lv_obj_t *btn_config = lv_btn_create(content);
 ...
 void app_main(void)
 {
     ...
     if (lvgl_lock(-1)) {
         ...
         create_main_screen();  // Crea la pantalla principal
          //   create_general_config_screen();  // Muestra el tabview con las pestañas y la configuración de productos en la 2ª pestaña
         lvgl_unlock();
     }
